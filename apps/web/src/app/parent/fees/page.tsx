...
export default function ParentFeesPage() {
  ...
  return (
    <div className="p-8 max-w-5xl mx-auto space-y-8">
      <div>
        <h1 className="text-3xl font-bold text-slate-900">Children's Fee Status</h1>
        <p className="text-slate-500">View and track fee records for your children.</p>
      </div>
      ...
      <div className="bg-white rounded-xl border border-slate-200 overflow-hidden shadow-sm">
        <table className="w-full text-left text-sm">
          ...
          <tbody className="divide-y divide-slate-100">
            {loading ? (
              <tr><td colSpan={6} className="text-center py-12 text-slate-500">Loading fees...</td></tr>
            ) : fees.length === 0 ? (
              <tr><td colSpan={6} className="text-center py-12 text-slate-500">No fee records found.</td></tr>
            ) : fees.map(fee => (
              <tr key={fee.id} className="hover:bg-slate-50 transition-colors">
                <td className="px-6 py-4 font-semibold text-primary">{fee.student?.first_name} {fee.student?.last_name}</td>
