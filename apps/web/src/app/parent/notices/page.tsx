...
export default function ParentNoticesPage() {
  ...
  return (
    <div className="p-8 max-w-5xl mx-auto space-y-8">
      <div>
        <h1 className="text-3xl font-bold text-slate-900">Parent Notice Board</h1>
        <p className="text-slate-500">Official circulars, updates, and notices from the school administration.</p>
      </div>
      ...
      <div className="grid gap-6">
        {loading ? (
          ...
          <div key={notice.id} className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm relative overflow-hidden">
            <div className={`absolute top-0 left-0 w-1.5 h-full ${notice.category === 'EMERGENCY' ? 'bg-red-500' : 'bg-emerald-500'}`} />
